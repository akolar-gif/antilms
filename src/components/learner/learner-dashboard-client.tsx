...
import { Sparkles, Brain, Quote, ArrowRight } from "lucide-react";
import { GDPRControls } from "@/components/learner/gdpr-controls";
import { GrowthProfile } from "@/components/learner/growth-profile";
import { AIImpulseCard } from "@/components/learner/ai-impulse-card";
import { I } from "@/components/layout/static-icons";
...
interface LearnerDashboardClientProps {
  ...
  bookedProgresses: any[];
  activeCourseProgress: any;
...
export function LearnerDashboardClient({
  ...
  const t = (key: string, params?: Record<string, string>) => {
    let text = translationsDict[key] || translationsDict.de?.[key] || String(key);
    if (params) {
      ...
      <div className="pad mb-6" style={{ marginTop: 20 }}>
        <div className="flex gap-2 pb-px" style={{ display: "flex", gap: 16, borderBottom: "1px solid var(--line)" }}>
          <button 
            ...
            className="pb-3 text-xs font-mono uppercase tracking-wider font-bold transition-all border-none bg-transparent cursor-pointer"
            style={{ 
      ...
      {activeTab === "growth" ? (
        ...
        <>
          ...
          <div className="pad animate-reveal" style={{ marginBottom: 30 }}>
            <AIImpulseCard lang={lang} reflectionsCount={reflections.length} />
          </div>
