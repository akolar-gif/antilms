...
import { Briefcase, Send, ShieldCheck, Clock } from "lucide-react";
...
export function ProjectTaskBlock({ block, onComplete }: { block: LearningBlock; onComplete?: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    setIsSubmitted(true);
    toast.success("Project submitted! Added to your learning portfolio. 🎉");
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm my-8">
      ...
      <div className="p-4 bg-royal-blue/5 border-b border-royal-blue/10 flex items-center justify-between">
        <div className="flex items-center space-x-2 text-royal-blue">
          ...
          <span className="font-semibold text-sm uppercase tracking-wider">Project Challenge</span>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Your Objective</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
          <div>
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Required Deliverable</h4>
        ...
        {!isSubmitted ? (
          <div className="space-y-4 border-t border-slate-100 pt-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                Submit Your Solution / Link to Deliverable
              </label>
              <textarea
                value={solution}
                onChange={(e) => setSolution(e.target.value)}
                placeholder="Paste your solution details, project description, or working link here..."
                rows={4}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                Post-Project Reflection: {taskData.reflectionPrompt}
              </label>
              <textarea
                value={reflection}
                onChange={(e) => setReflection(e.target.value)}
                placeholder="What was the most challenging part of this project, and what would you do differently next time?"
                rows={3}
                className="w-full p-3 border border-slate-200 rounded-xl outline-none text-sm focus:border-royal-blue focus:ring-1 focus:ring-royal-blue"
              />
            </div>
            ...
            <div className="flex justify-end">
              ...
              >
                <Send className="w-4 h-4 mr-2" /> Submit Project Portfolio
              </Button>
            </div>
          </div>
        ) : (
          <div className="bg-emerald-green/10 text-emerald-green p-6 rounded-xl border border-emerald-green/20 text-center space-y-3 flex flex-col items-center">
            ...
            <h4 className="font-heading font-bold text-lg">Project Portfolio Updated!</h4>
            <p className="text-sm opacity-90 max-w-md">
              Your solution and reflection have been successfully saved. This artifact is now part of your capability portfolio.
            </p>
            <div className="w-full bg-white p-4 rounded-lg text-left text-xs border border-emerald-green/15 text-slate-600 mt-2">
              <strong>Your submitted deliverable:</strong>
              <p className="mt-1 font-mono text-slate-700">{solution}</p>
              <strong className="mt-3 block">Your reflection:</strong>
